...
const SignUp = ({ navigation }) => {
  ...
  const handleSignUp = (credentials, setSubmitting) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        localStorage.setItem('doctorname', data.name,24 * 60 * 60 * 1000);
        console.log('local name', localStorage.getItem('doctorname'));
        // setUser(data._id)
  ...
  const validate = (values) => {
    ...
    if (!values.fullName) {
      ...
      errors.fullName = 'Full Name too short.Minimum Length is 8.';
    }
  ...
  return (
    <StyledContainer>
      ...
      <InnerContainer>
        ...
        <Formik
          initialValues={{ fullName: '',experience:'',speciality:'',price:'', email: '', password: '', confirmPassword: '' }}
          validate={validate}
          onSubmit={(values, { setSubmitting }) => {
            if (values.fullName == '' && values.experience == '' && values.speciality == '' && values.price == '' && values.email == '' && values.password == '' && values.confirmPassword == '') {
          ...
          {({ handleChange, handleBlur, handleSubmit, values, isSubmitting, errors, touched }) => (
            <StyledFormArea>
              ...
              {errors.experience && touched.experience && (
                <span style={{ color: 'red', fontSize: '16', marginTop: '5' }}>{errors.experience}</span>
              )}
              <MyTextInput
                label="Speciality"
                icon="person"
              ...
              {errors.speciality && touched.speciality && (
                <span style={{ color: 'red', fontSize: '16', marginTop: '5' }}>{errors.speciality}</span>
              )}
              <MyTextInput
                label="Price"
                icon="person"
              ...
              {errors.price && touched.price && (
                <span style={{ color: 'red', fontSize: '16', marginTop: '5' }}>{errors.price}</span>
              )}
              <MyTextInput
                label="Email Address"
                icon="mail"
